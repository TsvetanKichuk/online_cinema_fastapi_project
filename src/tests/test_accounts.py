...
from src.database.models.accounts import (
    User, UserGroup, ActivationToken, UserGroupEnum, PasswordResetToken, RefreshToken
)
...
def test_register_user_success(client, db_session, seed_user_groups):
    ...
    activation_token = db_session.query(ActivationToken).filter_by(user_id=created_user.id).first()
...
def test_activate_account_success(client, db_session, seed_user_groups):
    ...
    user = db_session.query(User).filter_by(email=registration_payload["email"]).first()
    ...
    token = db_session.query(ActivationToken).filter_by(user_id=user.id).first()
...
def test_activate_user_with_expired_token(client, db_session, seed_user_groups):
    ...
    activation_token = db_session.query(ActivationToken).filter_by(user_id=user.id).first()
...
def test_request_password_reset_token_success(client, db_session, seed_user_groups):
    ...
    reset_token = db_session.query(PasswordResetToken).filter_by(user_id=user.id).first()
...
def test_request_password_reset_token_nonexistent_user(client, db_session):
    ...
    reset_token_count = db_session.query(PasswordResetToken).count()
...
def test_request_password_reset_token_for_inactive_user(client, db_session, seed_user_groups):
    ...
    created_user = db_session.query(User).filter_by(email=registration_payload["email"]).first()
...
def test_reset_password_success(client, db_session, seed_user_groups):
    ...
    reset_token_record = db_session.query(PasswordResetToken).filter_by(user_id=created_user.id).first()
...
def test_reset_password_expired_token(client, db_session, seed_user_groups):
    ...
    expired_token = db_session.query(PasswordResetToken).filter_by(user_id=user.id).first()
...
def test_login_user_success(client, db_session, jwt_manager, seed_user_groups):
    ...
    user_group = db_session.query(UserGroup).filter_by(name=UserGroupEnum.USER).first()
    ...
    refresh_token_record = db_session.query(RefreshToken).filter_by(user_id=user.id).first()
