...
from src.database.models.accounts import User, UserProfile
...
def test_create_user_profile_with_fake_s3(
    ...
    profile_in_db = db_session.query(UserProfile).filter_by(user_id=user.id).first()
...
def test_admin_creates_user_profile(
    ...
    profile_in_db = db_session.query(UserProfile).filter_by(user_id=regular_user.id).first()
...
def test_user_cannot_create_another_user_profile(
    ...
    profile_in_db = db_session.query(UserProfile).filter_by(user_id=user_2.id).first()
...
def test_inactive_user_cannot_create_profile(
    ...
    user = User.create(email="inactive@mate.com", raw_password="TestPassword123!", group_id=1)  # 1 = User
...
def test_cannot_create_profile_twice(
    ...
    profiles_count = db_session.query(UserProfile).filter_by(user_id=user.id).count()
